...
SELECT
    ...
    (inflation_rate - LAG(inflation_rate) OVER (PARTITION BY country ORDER BY date)) / LAG(inflation_rate) OVER (PARTITION BY country ORDER BY date) * 100 AS mom_change,

    -- Year-over-year % change
    (inflation_rate - LAG(inflation_rate, 12) OVER (PARTITION BY country ORDER BY date)) / LAG(inflation_rate, 12) OVER (PARTITION BY country ORDER BY date) * 100 AS yoy_change,
